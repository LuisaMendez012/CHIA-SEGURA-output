(function () {
  ...
  function normalizeStatus(raw) {
    ...
    // Fallback por valores comunes del mock
  ...
  function toISODateParts(dateStr) {
    // dateStr esperado: en heatmap.js viene como "10 ene 2025, 14:30"
    // en modal.js guardan ts (Date.now) + campos sin ese formato.
    const months = {
    ...
    // Intentar parsear: "DD mon YYYY, HH:MM"
  ...
  function normalizeIncident(raw) {
    ...
    // Detectar formato de modal.js
    // - modal.js usa: name, last, cedula, email, type, address, desc, lat, lng, ts
    // - heatmap.js usa: type, address, description, lat, lng, date, status
  ...
  function computeStats(list) {
    ...
    const activeCategories = new Set(list.map(i => i.type).filter(Boolean)).size;

    // Últimos 7 días (si dateObj existe)
    const now = Date.now();
    const weekAgo = now - 7 * 86400000;
    const last7 = list.filter(i => i.dateObj && i.dateObj.getTime() >= weekAgo).length;

    // Incidentes recientes: tomamos top 4 por fecha
    const recent = [...list]
      .sort((a, b) => (b.sortDate || 0) - (a.sortDate || 0))
      .slice(0, 4);

    return { total, activeCategories, last7, recent };
  }

  function renderStats() {
    // Intencionalmente removido: ya no existen tarjetas/cuadros de estadísticas en la UI.
  }


  function statCard(label, value, helper) {
    // helper opcional para tooltips
    const hint = helper ? `title="${escapeHtml(helper)}"` : '';
    return `
      <div class="hero-mini-stat" ${hint}>
        <div class="hero-mini-stat__num">${escapeHtml(String(value))}</div>
        <div class="hero-mini-stat__label">${escapeHtml(label)}</div>
      </div>
    `;
  }
  ...
  function renderTable() {
    ...
    tbody.querySelectorAll('.view-more').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // Simple modal-like using alert (sin saturar UI). Se puede mejorar después.
        // Si quieres modal premium en otra iteración, lo integro.
  ...
  function showToastModal(title, htmlEncoded) {
    // Usamos un modal mínimo para no romper diseño existente.
    // toast.js existe, pero aquí necesitamos contenido largo.
    const msg = document.createElement('div');
    msg.innerHTML = htmlEncoded;
    ...
    overlay.innerHTML = `
      <div style="max-width:720px; width:100%; background:#fff; border-radius:16px; border:1px solid rgba(148,163,184,0.35); box-shadow:0 20px 60px rgba(0,0,0,0.25);">
        ...
        <div style="padding:14px 16px; color:#0f172a; font-weight:800; max-height:60vh; overflow:auto; line-height:1.4;">
          ${htmlEncoded}
        </div>
  ...
  function requireExportLibs() {
    ...
    // autoTable (puede registrarse en distintos lugares dependiendo del build/UMD)
    const hasAutoTable =
  ...
  function generateExcel(list, filename) {
    ...
    // XLSX+saveAs son necesarias para Excel
  ...
  async function generatePDF(list, filterSummary) {
    ...
    // Normalización básica para evitar render raro de Unicode en algunos navegadores.
    // (Mantiene UTF-8 en la mayoría, pero evita separaciones/diacríticos corruptos.)
    ...
    // DEBUG: verificar tipo real del string antes de render
    console.log('[reportes.js] typeof filterSummary:', typeof filterSummary, 'value:', filterSummary);
    console.log('[reportes.js] typeof filterLine:', typeof filterLine, 'value:', filterLine);

    // Prueba visual simple (para comparar render)
    ...
    // autoTable puede no estar en doc, según cómo se registró el plugin.
    // Intentamos llamar donde esté disponible.
    const autoTableFn =
    ...
    // Normaliza textos para evitar problemas de render (diacríticos/símbolos raros)
  ...
  function init() {
    ...
    // renderStats(); // ya no se renderizan tarjetas/cuadros de estadísticas
    renderTable();
